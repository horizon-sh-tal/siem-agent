"""Windows Event Log collector for SIEM Agent.

Reads Windows Event Log channels defined in config.json, tracks last
record ID via CheckpointManager, encrypts batches with RSA-4096 hybrid
encryption, and ships to Kafka topics named: {machine_id}-{log_key}
"""
...
class WindowsLogCollector:
    ...
    def run_collection_cycle(self) -> None:
        """Execute one collection cycle for all enabled event log sources."""
        ...
        for log_key, log_cfg in logs_cfg.items():
            if not log_cfg.get("enabled", False):
                continue
            try:
                self._collect_event_log(log_key, log_cfg)
    ...
    def _collect_event_log(self, log_key: str, log_cfg: Dict[str, Any]) -> None:
    ...
    def _query_events(
        ...
        ps_script = (
            f"try {{\n"
            f"  $events = Get-WinEvent -FilterHashtable @{{{filter_hashtable}}}"
            f" -ErrorAction Stop -MaxEvents {self.MAX_EVENTS_PER_BATCH * 2}\n"
            f"  $events | Where-Object {{ $_.RecordId -gt {after_record_id} }} |\n"
            f"    Select-Object RecordId, TimeCreated, Id, LevelDisplayName,"
            f" ProviderName, Message"
            f" -First {self.MAX_EVENTS_PER_BATCH} |\n"
        ...
        # PowerShell returns a dict (not list) when only one event is returned
        if isinstance(parsed, dict):
            parsed = [parsed]

        return parsed

    def _build_payload(self, log_key: str, events: List[Dict[str, Any]]) -> str:
