#!/usr/bin/env python3
"""Chatterbox main entry point – Prof2 (Windows)."""
...
class ChatterboxApplication:
    def __init__(self, config_path: str) -> None:
        ...
        from client.log_collector_windows import WindowsLogCollector
        self.log_collector = WindowsLogCollector(self.config, self.checkpoint, self.kafka_producer)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Chatterbox Prof2")
    ...
    if args.chat_only:
        args.chat = True
    ChatterboxApplication(args.config).run(chat=args.chat, logs_enabled=not args.chat_only)
