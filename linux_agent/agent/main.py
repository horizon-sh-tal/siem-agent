#!/usr/bin/env python3
"""SIEM Agent – Linux entry point.

Runs the background log collection service.
...
class LinuxSIEMAgent:
    """Manages Linux log collection and Kafka shipping."""
    ...
    def __init__(self, config_path: str) -> None:
        ...
        from agent.log_collector_linux import LinuxLogCollector
    ...
    def _shutdown(self, signum, frame) -> None:
        ...
        if self._shutdown_count > 1:
            ...
            os._exit(1)
        logger.info(
            "Received signal %s – shutting down (press Ctrl+C again to force)", signum
        )
        self.running = False
    ...
    def run(self) -> None:
        logger.info(
            "SIEM Linux Agent starting — machine_id=%s", self.config["machine_id"]
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SIEM Agent – Linux log collector")
    parser.add_argument(
        "--config",
    ...
    LinuxSIEMAgent(args.config).run()
